...
export default function Ruler({ clips, pps }) {
  ...
  return (
    <div className="relative h-4 bg-neutral-900 border-b border-neutral-800" style={{ width: Math.max(totalWidth, 1), minWidth: '100%' }}>
      {ticks.map(t => (
        <div key={t} className="absolute top-0 bottom-0" style={{ left: t * pps }}>
          <div className={`w-px bg-neutral-700 ${t % 5 === 0 ? 'h-2' : 'h-1'}`} />
          {t % 5 === 0 && (
            <span className="absolute left-1 top-0 text-[8px] text-neutral-500 leading-none">{t}s</span>
          )}
        </div>
      ))}
    </div>
