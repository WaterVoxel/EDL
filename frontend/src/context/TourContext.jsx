...
export const TOUR_STEPS = [
  ...
  { id: 'timeline', text: "The Timeline tab: V1 is your main edit track, V2 is a scratch lane for Analyze/Reconstruct, and A1 underneath holds a single audio bed that stays locked to V1 — padded or cut to the sequence on every render. The Edit Decision List below it lists every clip's decisions." },
  { id: 'renderBar', text: 'Everything that runs ffmpeg sits in this row. Left: V2 Reconstruct and V2 Analyzer, which rewrite clips rather than write a file. Center: the transport — play/stop, frame stepping, first/last, loop, and a clock you can type into (click it to switch timecode/frames). Right, in track order: Render V1 applies every V1 decision in one pass, Render V2 writes the V2 track (its A / A/B switch picks V2 alone or V2 composited over V1), and Render A1 writes the audio alone as a .wav the same length as the V1 render. The V2 and A1 buttons appear only once those tracks have something on them.' },
