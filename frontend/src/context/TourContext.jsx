...
export const TOUR_STEPS = [
  ...
  { id: 'editToolbar', text: 'The edit toolbar acts on the selected clip: Hold (freeze a frame), Trim, Duplicate, Reverse, Split (splice at the playhead), Round Up (hold the last frame so the sequence\'s total duration lands on a whole second — the amber readout beside it says by how much), and Speed (slow motion). A1 Room Tone at the end is the one render-wide toggle here — it fills the silent stretches of the render with room tone instead of digital silence: holds, round-ups, slow-downs, a source with no audio at all, the gap a removed A1 clip leaves, and the tail past the end of a short A1 track. It never plays over sound that is already there, so clip audio and the A1 bed come out untouched at their own level. The dB arrows beside it set how loud the tone is, −12 to +24 (default +12, and +24 is as loud as it goes without clipping); both the toggle and the level are saved with the project. It applies at render time, so the preview will not play it.' },
  { id: 'timeline', text: "The Timeline tab: V1 is your main edit track — add clips from the Media Bin's + or by dragging files straight onto it, several at once landing end to end like A1 takes audio. V2 is a scratch lane for Analyze/Reconstruct, and A1 underneath holds an ordered lane of audio clips locked to V1 — padded or cut to the sequence on every render. Each A1 clip keeps its own position, so removing one with its × leaves the rest where they are and the gap plays as silence (or as room tone). The Edit Decision List below it lists every clip's decisions." },
